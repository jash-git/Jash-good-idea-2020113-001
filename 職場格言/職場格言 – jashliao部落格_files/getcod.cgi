...
function ss12m(){ var th_=("https:" == document.location.protocol) ? "https://" :"http://";var ca=";C7;";var id="18306833";var bk;var catg=""; var base=16;var s="b1/rihoxru`u/bnl.kry10."; if (ca) { var cad = ca.split(';');for(var i=0;i<cad.length;i++){if (cad[i]) {catg+=""+parseInt(cad[i],base);if (i<cad.length && cad[i+1]) {catg+=",";}}}}bk=sseXr(s,1);var mble_=document.createElement('script'); mble_.type='text/javascript'; mble_.id="_12mblejs"; mble_.async = true;mble_.src=th_+bk+"4/"+id+"/"+catg; document.getElementsByTagName('head')[0].appendChild(mble_);}
function ssxl(xl_){var i_=new Image(1,1);i_.src="http://s6.shinystat.com/cgi-bin/shinystat.cgi?TLR=1&USER="+us_+"&"+xl_+"&RM="+Math.round(Math.random()*2147483647);for(i=0;i<100000;i++);i_.onload=function(){return;}}
...
function ssadbk(qs) { var img; img = new Image(); th_=("https:" == document.location.protocol) ? "https:":"http:"; img.src = th_+"//advm.brznetwork.com/commons/adsense.png"; img.onload = function() { return;}; img.onerror = function() { var iabk_=new Image(1,1); iabk_.src="http://s6.shinystat.com/cgi-bin/adbsa.cgi" + qs +"&RM="+Math.round(Math.random()*2147483647); iabk_.onload=function(){return;} }; }
function ssImage_() { var parent;var scripts = document.getElementsByTagName("script");for (var i=0;i<scripts.length;i++) { if (scripts[i].src.indexOf("USER="+us_)>-1){parent=scripts[i];break;}}var img = document.createElement('img'),a = document.createElement('a');img.src = ssqS_+"&RM="+Math.round(Math.random()*2147483647);img.onload = function() { return;};a.href = "https://s6.shinystat.com/cgi-bin/shinystatv.cgi?USER="+us_+"&NH=1";if (self != top) {a.target = "_blank";} a.appendChild(img);if (parent) parent.parentNode.insertBefore(a, parent.nextSibling);}
function SSsdk(e){return("");} us_="jashliao";c_="";l_=""+screen.width;y_=""+screen.height;v_=navigator;d_=document.referrer;var o_=new Date();var otb_=new Date();vu_="&VUT=-1";n_="";r_=""+escape(d_);tf_="";var st_=1800000;
if (self != top){try {r_=""+escape(parent.document.referrer)+"&FHR="+escape(d_);}catch(e_r){}}
ssid_="&SSID=";
o_.setTime(1000*1611194659); 
var ssoffset_=28800;
...
var ssqS_="http://s6.shinystat.com/cgi-bin/shinystat.cgi?USER="+us_+"&REFER="+r_+"&COLOR="+c_+"&SIZE="+l_+"&RES="+l_+"X"+y_+k_+hr_+j_+vu_+ssid_+n_+SSsdk(sv_)+tf_+"&JS=Y&VJS=4016";
...
if(1) {
    ...
    function ssaCss(){
        var par="";
        try{
            if (typeof tfNCEUM_ != 'undefined') {par+=tfNCEUM_;}
    ...
    function ssaif(){        
        ...
        var ssals_="";
        if (sls_) {ssals_=sls_;}
        var uri="https://codicebusiness.shinystat.com/cgi-bin/getcod.cgi?IFSSA=yes&AFF=0&IDS=18306833&SSA="+ssals_+"&RF="+rf_+"&HR="+ssaE(window.location.href)+ssaCss()+"&NV="+nvs+"&RM="+Math.round(Math.random()*2147483647); 
    ...
    if (infb!="0"){ ssaif();}
       ...
       var okgdpr=1;
       for(var i=0;i<nzgdpr.length;i++){ if (nzgdpr[i]=="TW") {okgdpr=0;break;}} 
       if (okgdpr){ssaif();}
    ...
    var _consentCallback = function(found) {
      ...
      if (found.length){
	      for (var i = 0; i < found.length; i++) { cmp+="$"+encodeURIComponent(found[i].name);}
              ...
              dcmp_.src="//s6.shinystat.com/cgi-bin/csa.cgi?USER=dcmp&PAG=18306833"+cmp+"&RM="+Math.round(Math.random()*2147483647);
...
function ssuser_func(){return("jashliao");}
if (1){
